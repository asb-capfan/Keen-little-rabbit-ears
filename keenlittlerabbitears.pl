...
#
# Written by Matthias Nutt (1999).
#
# Slightly modified by Alex.
#
...
# -------------------- BEGIN: THE PLATFORM INDEPENDENT SECTION -------------

# A temporary file will be created and then played by timidity.
# For this, we need a temp directory and a temp file.
my $tmp_midi_dir = File::Temp->newdir();
my $tmp_midi_fh  = File::Temp->new(
    TEMPLATE => 'keenlittlerabbitearsXXXXXX',
    DIR      => $tmp_midi_dir,
    SUFFIX   => '.midi',
);

# The name of the temporary MIDI file
my $tmp_midi_file = File::Spec->catfile($tmp_midi_dir, $tmp_midi_fh->filename);

# the  keen little rabbit ears configuration file
# Full path to file
my $configuration_file = File::Spec->catfile($Bin, 'keenlittlerabbitears.cfg');

# -------------------- END: THE PLATFORM INDEPENDENT SECTION ---------------
...
"Warning:\nYou have to select at least one item!\nClick Ok to continue"
    ...
    foreach $key (keys %$act_lists) {
        $act_lists->{"$key"}->{"checkbutton"}->configure(-state => "disabled");
    }
...
sub select_instrument {
    ...
    $instrument_listbox = $instrument_listbox_frame->Listbox(
        -height     => "10",
        -selectmode => "single"
    );
    ...
    $instrument_scrollbar =
      $instrument_listbox_frame->Scrollbar(
        -command => [ "yview", $instrument_listbox ]);

    $instrument_listbox->configure(
        -yscrollcommand => [ "set", $instrument_scrollbar ]);
    ...
    $select_instrument_window->Button(
        -text    => "OK",
        -command => sub {
...
sub select_root_tone {
    ...
    $select_root_tone_window = $main_window->Toplevel(-title => "Select root");

    my $root_tone_listbox_frame = $select_root_tone_window->Frame();
    $root_tone_listbox_frame->pack();

    my $root_tone_listbox = $root_tone_listbox_frame->Listbox(
        -height     => "10",
        -selectmode => "single"
    );
    ...
    for ($i = $deepest_note ; $i < $highest_note ; $i++) {
    ...
    my $root_tone_scrollbar =
      $root_tone_listbox_frame->Scrollbar(
        -command => [ "yview", $root_tone_listbox ]);

    $root_tone_listbox->configure(
        -yscrollcommand => [ "set", $root_tone_scrollbar ]);
    ...
    # OK button
    $select_root_tone_window->Button(
        -text    => "OK",
        -command => sub {
            if (defined $root_tone_listbox->curselection()) {
                $selected_value = $root_tone_listbox->curselection();
                if ($selected_value == ($random_entry - $deepest_note)) {
                    $use_random_root = $on;
                } else {
                    $default_root    = $selected_value + $deepest_note;
                    $use_random_root = $off;
                }
            }
...
sub exercise {
    ...
    if (Exists($drill_window)) {
        ...
        $drill_window =
          $main_window->Toplevel(
            -title => "Keen Little Rabbit Ears Drill Window");
    }
    $drill_frame_top = $drill_window->Frame();
    $drill_frame_top->pack(
        -side   => "top",
        -fill   => "both",
        -expand => "yes"
    );
    $drill_frame_bottom = $drill_window->Frame();
    $drill_frame_bottom->pack(
        -side   => "bottom",
        -fill   => "both",
        -expand => "yes"
    );
    $drill_frame_left = $drill_window->Frame();
    $drill_frame_left->pack(
        -side   => "top",
        -fill   => "both",
        -expand => "yes"
    );
    ...
    foreach $element (
        sort {
            $act_chords->{$a}->{"sortkey"} <=> $act_chords->{$b}->{"sortkey"}
        }
        keys %$act_chords
      ) {
        ...
        $act_checkbutton->configure(
            -variable => \$act_chords->{$element}->{"status"},
            -onvalue  => $on,
            -offvalue => $off,
            -command  => [ \&change_button_state, $act_chords->{$element} ]
        );
        $act_checkbutton->pack(-side => "left");

        # use declared Button
        $act_button->configure(
            -text    => "$element",
            -command => [ \&check_answer, $i, $act_chords ]
        );
        $main_window->bind(
            $act_button,
            "<3>",
            sub {
                play_tone_list(\$act_chords->{$element}->{"notes"});
            }
        );
...
sub add_labels_and_buttons_on_bottom {
    ...
    $answer_label = $drill_frame_bottom->Label(
    ...
    $next_button = $control_button_frame->Button(
        -text    => "Next",
    ...
    $play_it_again_button = $control_button_frame->Button(
        -text    => "Play it again",
        -command => [ \&play_tone_list, \$tone_list ],
        -state   => "disabled"
    );
    $play_it_again_button->pack(
        -fill   => "both",
        -expand => "no"
    );

    $control_button_frame->Button(
        -text => "Stop exercise",
    ...
    $volume_frame->pack();
    $volume_frame->Label(-text => "Volume :")
      ->pack(-side => "left", -fill => "both", -expand => 1);
    $volume_frame->Radiobutton(
        -variable => \$volume,
        -value    => $volume_piano,
        -text     => "p"
    )->pack(-side => "left", -fill => "both", -expand => 1);
    ...
    if (   ($act_chords != $tones)
        ...
        $play_mode_frame->Label(-text => "Play it :")->pack(
            -side   => "left",
            -fill   => "both",
            -expand => "yes"
        );
        ...
        $play_direction_frame->Label(-text => "Direction :")->pack(
            -side   => "left",
            -fill   => "both",
            -expand => "yes"
        );
    ...
    $drill_frame_bottom->Checkbutton(
        -text     => "Quiz mode",
...
sub read_configuration_file {
    ...
    while (defined($line = <IN>)) {
        ...
        if (not($tmp_value =~ m/status/i)) {
            die(
                "Error in configuration file $configuration_file",
        ...
        if (not exists $all_tone_lists->{$identifier}) {
            ...
            $all_tone_lists->{$identifier}->{"sortkey"} = $identifier_counter++;
        ...
        if (
            not
            exists ${ $all_tone_lists->{$identifier}->{"tonelists"} }{$name}) {
...
foreach $element (
    ...
    $main_window->Button(
        -text    => "$element",
        -command => [ \&exercise, $all_tone_lists->{$element}->{"tonelists"} ]
